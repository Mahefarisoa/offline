let lotData = []; // Variable globale pour stocker les données
var baseUrl = 'http://localhost:3069/json-files';
// var baseUrl = 'https://www.tridisphere.fr/api-2/json-files';
var projectName = 'Loyettes'
...
async function fetchData() {
    try {
        const response = await fetch(`${baseUrl}/${projectName}`, {
            headers: {
                'accept': '*/*',
                'x-api-key': 'T6pXt9efz6OyGS272HnQx1Fh'
            }
        });
        if (!response.ok) {
            throw new Error('Échec du chargement des données');
        }
        lotData = await response.json();
        console.log('Data dans back:', lotData)
    } catch (error) {
...
function object2vrPlayer(A) {
    ...
    async function initialize() {
        await fetchData(); // Assurez-vous que les données sont chargées avant d'exécuter Jb

        // Exemple d'utilisation de Jb avec les données chargées
        // Vous pouvez appeler Jb pour différents IDs en fonction de vos besoins
        if (lotData && lotData.data) {
            // Itérer sur les IDs disponibles dans vos données et appeler Jb pour chaque
            Object.keys(lotData.data).forEach(id => {
                Jb({
                    id: id, // ID réel à partir des données
                    target: lotData.data[id].type // Exemple de target, ajustez en fonction des données
                });
            });
        } else {
            console.error("Les données ne sont pas disponibles ou mal formatées.");
        }
    }

    // Appelez initialize au chargement de la page ou lors de l'événement souhaité
    initialize();
    ...
    function Jb(a) {
        ...
        const currentLot = lotData.data[a.id];
        if (!currentLot) {
            console.error("Lot non trouvé pour l'ID:", a.id);
            return;
        }
        ...
        if (['301', '302', '303', '304'].includes(a.id)) {
        ...
        if (['301', '302', '303', '304'].includes(a.id)) {
            disabledClass = ' btn-outline-primary btn-show';
        }
  ...
  ========================== */
  let statut_class = "";
  let statutBackgroundColor = "#FFFFFF";

  switch (currentLot.statut) {
      case "Disponible":
          statut_class = "bg-customD";
          statutBackgroundColor = "#00ea8b";
          break;
      case "Indisponible":
          statut_class = "bg-custom";
          statutBackgroundColor = "#979696";
          displayClass3D = ' btn-outline-secondary disabled';
          break;
      default:
          statut_class = "bg-" + currentLot.statut.toLowerCase();
  }




            (l.innerHTML = '<div class="bg-blue-marine pb-0" style="border-radius: 8px;"><span class="h4">' + a.target + '&nbsp;</span><span style="font-size: .8rem">Lot ' + a.id + '</span><p class="mb-0 text-uppercase text-white bg-' + statut_class + ' pb-2 pt-2" style="font-size: .8rem; background-color:' + statutBackgroundColor +'">'+ currentLot.statut +'</p></div>' +
    ...
    function Ka(a, e) {
        if (c.inactiveHotspots.indexOf(c.hotspot.target) < 0) {
            ...
            if (b.enabled && (c.hotspot != c.emptyHotspot && 0 <= a && 0 <= e && "" != c.hotspot.title)) {
                ...
                const currentLot = lotData.data
                ? lotData.data[c.hotspot.title]
                : lotData[c.hotspot.title];
            ...
            if (!currentLot) return;
                ...
                const allowedIds = [
                    '301','302','303','304'
                            ...
                            if (allowedIds.includes(c.hotspot.title)) {
                disabledClass = ' btn-outline-primary btn-showVV';
                displayClass3D = ' btn-outline-primary btn-show';
                displayClass2D = ' btn-outline-primary btn-show';
            } else {
                disabledClass = displayClass3D = displayClass2D = ' d-none';
            }
            ...
            switch (currentLot.statut) {
                ...
                case "Indisponible":
                    statut_class = "bg-custom";
                    statutBackgroundColor = "#979696";
                    disabledClass = displayClass2D = displayClass3D = ' btn-outline-secondary disabled';
                    break;
                default:
                    statut_class = "bg-" + currentLot.statut.toLowerCase();
            }

                l.innerHTML = '<div class="bg-blue-ciel pb-0" style="border-radius: 8px;"><span class="h4">' + currentLot.type + '&nbsp;</span><span style="font-size: .8rem">Lot ' + currentLot.label + '</span><p class="mb-0 text-uppercase text-white bg-' + statut_class + ' pb-2 pt-2" style="font-size: .8rem; background-color:'+ statutBackgroundColor +'">' + currentLot.statut + '</p></div>' +
                    '<div class="bg-white mt-1">' +
                    '<button class="btn btn-outline-primary w-75 mb-2 mt-2 ' + displayClass2D + '" data-cat="2" data-set="' + currentLot.title + '" style="font-size:.9rem;">2D</button>' +
...
`;
    ...
    function showVV() {
        console.log('showVV called');
        $(document).off("click", "button.btn-showVV").on("click", "button.btn-showVV", function() {
            const currentlot = $(this).attr("data-set");
            let urlvv = '';
            if (currentlot == 'A301') {
                urlvv = 'https://panoraven.com/fr/embed/YYvxlmUoft';
            } else if (currentlot == '202') {
                urlvv = 'https://panoraven.com/fr/embed/7n2hg2FZ5J';
            }


            const currentVV = `<iframe width="95%" class="iframevv" height="600vh" allowFullScreen="true" allow="accelerometer; magnetometer; gyroscope" src="${urlvv}"></iframe>`;
    ...
    function ec() {
        ...
        img.style.transform = "rotateX(0deg) rotateZ(" + (110 + (f.I * 360) / 25) + "deg)";
        if (ca) {
            if (((ya = !1), 0 <= f.I && 0 <= f.J)) {
                (b < D || g < E) &&
                Ha > n &&
