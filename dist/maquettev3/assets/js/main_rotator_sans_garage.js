...
function oc(A) {
    if (A) {
        for (
            var U = nc, m = ["Array", "prototype", "fill"], z = 0; z < m.length - 1; z++
        ) {
            var Q = m[z];
            Q in U || (U[Q] = {});
            U = U[Q];
...
function object2vrPlayer(A) {
    ...
    function Jb(a) {
        ...
        if (b.enabled && 0 <= mx && 0 <= my) {
            let disabledClass = ' btn-outline-secondary disabled';
            let displayClass3D = ' btn-outline-secondary disabled';
            let displayClass2D = ' btn-outline-secondary disabled';
            if (null) {
                disabledClass = ' btn-outline-primary btn-showVV'
            } else {
                disabledClass = ' d-none'; // Masquer le bouton 3D si l'ID n'est pas dans la liste spécifiée
            }

            if (a.id === '106' || a.id === '204' || a.id === '301' || a.id === '402') {
                displayClass3D = ' btn-outline-primary btn-show';
            } else {
                displayClass3D = ' d-none'; // Masquer le bouton 3D si l'ID n'est pas dans la liste spécifiée
            }

            if (a.id === '101' || a.id === '102' || a.id === '103' || a.id === '104' || a.id === '105' || a.id === '106' || a.id === '107' ||
                a.id === '201' || a.id === '202' || a.id === '203' || a.id === '204' || a.id === '205' || a.id === '206' || a.id === '207' ||
                a.id === '301' || a.id === '302' || a.id === '303' || a.id === '304' || a.id === '305' || a.id === '306' || a.id === '307' ||
                a.id === '401' || a.id === '402' || a.id === '403' || a.id === '404' || a.id === '405' || a.id === '406' || a.id === '407'
            ) {
                displayClass2D = ' btn-outline-primary btn-show';
            } else {
                displayClass2D = ' d-none'; // Masquer le bouton 3D si l'ID n'est pas dans la liste spécifiée
            }

            // Indispo status
            // TGBT01 || 001 || 002 || 003 || 004 || 005 || 006 || com01 || com03 || com04 || NGF || OM01 || OM02 || velo || encombrants
            // const indispoStat = ['TGBT01',  '001' , '002' , '003' , '004' , '005' , '006' , 'com01' , 'com03' , 'com04' , 'NGF' , 'OM01' , 'OM02' , 'velo' , 'encombrants']
            const indispoStat = []
            let lStatus = 'Disponible'
            let lStatusClass = 'disponible'
            if (indispoStat.includes(a.id)) {
                lStatus = 'Indisponible'
                lStatusClass = 'indisponible'
            }


            (l.innerHTML = '<div class="bg-blue-marine pb-0" style="border-radius: 8px;"><span class="h4">' + a.target + '&nbsp;</span><span style="font-size: .8rem">Lot ' + a.id + '</span><p class="mb-0 text-uppercase text-white bg-' + lStatusClass + ' pb-2 pt-2" style="font-size: .8rem">' + lStatus + '</p></div>' +
                '<div class="bg-white mt-1" style="border-radius: 8px;">' +
                '<button class="btn btn-outline-primary w-75 mb-2 mt-2 ' + displayClass2D + '" data-cat="2" data-set="' + a.id + '" style="font-size:.9rem;">2D</button>' +
                '<button class="btn btn-outline-primary w-75 mb-2 btn-show' + displayClass3D + '" data-cat="3" data-set="' + a.id + '" style="font-size:.9rem;">3D</button>' +
                // '<button class="btn w-75 mb-2' + disabledClass + '" data-set="' + a.id + '" style="font-size:.7rem;">Visite Virtuelle</button>' +
                '<br/>' +
                '</div>'
            ),
            // Styles additionnels pour l'élément l
            l.style.visibility = "inherit";
            l.style.position = "absolute";
            l.style.backgroundColor = b.background ? "white" : "transparent";
            l.style.textAlign = "center";
            l.style.padding = "0px";
            l.style.borderRadius = "8px";
            l.style.boxShadow = "2px 2px 7px 2px rgba(0, 0, 0, .4)";
            l.style.color = "white";

            // Ajouter la flèche (en utilisant :after pour le positionnement dynamique)
            l.style.position = "relative";
            l.classList.add("tooltipes_");

            // Supprimer la valeur fixe de 210 et utiliser la hauteur réelle du popup
            const rect = l.getBoundingClientRect();
            const popupHeight = rect.height || 100; // Hauteur par défaut plus petite maintenant

            // Position verticale initiale : placer le popup juste au-dessus du point (my)
            let topPosition = my - popupHeight; // petit espace de 10px entre la flèche et le lot
            let arrowPosition = "bottom";

            // Si le popup dépasse le haut de l’écran, on le met en dessous
            if (topPosition < 0) {
                topPosition = my + 20; // on le met 20px en dessous du point
                arrowPosition = "top";
            }

            // Position horizontale inchangée
            const leftPosition = mx - (b.width ? b.width / 3 : l.offsetWidth / 2);

            // Appliquer les positions recalculées
            l.style.top = `${topPosition}px`; // ✅ ne plus ajouter +90
            l.style.left = `${leftPosition}px`;


            // Ajouter la flèche dans le style du popup
            l.style.setProperty("--arrow-position", arrowPosition);

            // Ajustement de la largeur du popup
            l.style.width = b.width ? `${b.width / 1.4}px` : "auto";

            // Ajouter des styles pour la flèche
      ...
      .tooltipes_::after {
        ...
        border-top: ${arrowPosition === "bottom" ? '6px solid white' : 'none'}; /* Flèche pointant vers le lot en haut */
        border-bottom: ${arrowPosition === "top" ? '6px solid white' : 'none'}; /* Flèche pointant vers le lot en bas */
        top: ${arrowPosition === "top" ? `-6px` : `calc(100% + 2px)`}; /* Ajuster la position de la flèche */
      }
      `;
        ...
        if (
            ...
            if (J) {
                ...
                for (var e = 0; e < G.length; e++) {
                    ...
                    ///////////////////////////////////////////////////
                    if (
                        c.inactiveHotspots.indexOf(b.target) >= 0 ||
                        (c.toShow && !c.toShow[b.title])
                    ) {
                        // this mean that b is innactive
                        continue;
                    }
    ...
    function za(a, e, b) {
        ...
        if (c) {
            ...
            var f = b.toString();
            for (
                c.charAt(c.length - 1) != e && (f = (1 + b).toString()); f.length < c.length - 1;
    ...
    function Ka(a, e) {
        if (c.inactiveHotspots.indexOf(c.hotspot.target) < 0) {
            var b = h.v;
            if (b.enabled && (c.hotspot != c.emptyHotspot && 0 <= a && 0 <= e && "" != c.hotspot.title)) {
                let disabledClass = ' btn-outline-secondary disabled';
                let displayClass3D = ' btn-outline-secondary disabled';
                let displayClass2D = ' btn-outline-secondary disabled';
                if (null) {
                ...
                if (c.hotspot.title === '106' || c.hotspot.title === '204' || c.hotspot.title === '301' || c.hotspot.title === '402') {
                    displayClass3D = ' btn-outline-primary btn-show';
                } else {
                    displayClass3D = ' d-none'; // Masquer le bouton 3D si l'ID n'est pas dans la liste spécifiée
                }

                if (c.hotspot.title === '101' || c.hotspot.title === '102' || c.hotspot.title === '103' || c.hotspot.title === '104' || c.hotspot.title === '105' || c.hotspot.title === '106' || c.hotspot.title === '107' ||
                    c.hotspot.title === '201' || c.hotspot.title === '202' || c.hotspot.title === '203' || c.hotspot.title === '204' || c.hotspot.title === '205' || c.hotspot.title === '206' || c.hotspot.title === '207' ||
                    c.hotspot.title === '301' || c.hotspot.title === '302' || c.hotspot.title === '303' || c.hotspot.title === '304' || c.hotspot.title === '305' || c.hotspot.title === '306' || c.hotspot.title === '307' ||  
                    c.hotspot.title === '401' || c.hotspot.title === '402' || c.hotspot.title === '403' || c.hotspot.title === '404' || c.hotspot.title === '405' || c.hotspot.title === '406' || c.hotspot.title === '407'
                ) {
                    displayClass2D = ' btn-outline-primary btn-show';
                } else {
                    displayClass2D = ' d-none'; // Masquer le bouton 3D si l'ID n'est pas dans la liste spécifiée
                ...
                // Indispo status
                // TGBT01 || 001 || 002 || 003 || 004 || 005 || 006 || com01 || com03 || com04 || NGF || OM01 || OM02 || velo || encombrants
                // const indispoStat = ['TGBT01',  '001' , '002' , '003' , '004' , '005' , '006' , 'com01' , 'com03' , 'com04' , 'NGF' , 'OM01' , 'OM02' , 'velo' , 'encombrants', 'A102', 'S104', 'S105', 'S106', 'S107', 'S108', 'S206', 'S207', 'S208', 'S209', 'S210', 'S203', 'S204', 'A204', 'A202',
                //                      'S303', 'S205', 'adino','S304','S305','S306','S307', 'S308', 'A201', 'A202', 'A303', 'A302', 'A410', 'A412', 'A413', 'A414', 'A415', 'A416', 'A509', 'A511', 'A512', 'A513', 'A514', 'A515', 'A502']
                const indispoStat = [];
                let lStatus = 'Disponible'
                let lStatusClass = 'disponible'
                if (indispoStat.includes(c.hotspot.title)) {
                    lStatus = 'Indisponible'
                    lStatusClass = 'indisponible'
                }

                l.innerHTML = '<div class="bg-blue-ciel pb-0" style="border-radius: 8px;"><span class="h4">' + c.hotspot.target + '&nbsp;</span><span style="font-size: .8rem">Lot ' + c.hotspot.title + '</span><p class="mb-0 text-uppercase text-white bg-' + lStatusClass + ' pb-2 pt-2" style="font-size: .8rem">' + lStatus + '</p></div>' +
                    '<div class="bg-white mt-1">' +
                    '<button class="btn btn-outline-primary w-75 mb-2 mt-2 ' + displayClass2D + '" data-cat="2" data-set="' + c.hotspot.title + '" style="font-size:.9rem;">2D</button>' +
                    '<button class="btn w-75 mb-2 ' + displayClass3D + '" data-cat="3" data-set="' + c.hotspot.title + '" style="font-size:.9rem;">3D</button>' +
                    // '<button class="btn w-75 mb-2' + disabledClass + '" data-set="' + c.hotspot.title + '" style="font-size:.7rem;">Visite Virtuelle</button>' +
                    '<br/>' +
                    '</div>',
                    // Calculer la largeur et la position initiale
                ...
                if (b.width > 0) {
                    l.style.left = a - b.width / 3 + "px";
                    l.style.width = (b.width / 1.4) + "px";
                } else {
                    l.style.width = "auto";
                ...
                // l.style.height = "13rem";

                l.style.height = "auto";
                l.style.minHeight = "3rem";
                l.style.padding = "5px";
                l.style.top = e + -210 + "px"; // 210px de décalage

                // Vérifier si le popup dépasse la hauteur de l'écran et ajuster la position de la flèche
                (function() {
                    const rect = l.getBoundingClientRect(); // Dimensions actuelles du popup
                    const popupHeight = rect.height || 100; // Hauteur par défaut réduite
                    const viewportHeight = window.innerHeight; // Hauteur visible de l'écran

                    // Position initiale : au-dessus du point (e)
                    let topPosition = e - popupHeight; // on place le popup au-dessus avec un léger espace
                    let arrowPosition = "bottom"; // flèche en bas par défaut (popup au-dessus)

                    // Si le popup dépasse le haut de l'écran, on l'affiche en dessous
                    if (topPosition < 0) {
                        topPosition = e + 20; // un petit espace sous le point
                        arrowPosition = "top"; // flèche en haut si le popup est en dessous
                    }

                    // Appliquer la position recalculée
                    l.style.top = `${topPosition}px`;

                    // Ajouter un style dynamique pour la flèche
                    l.style.setProperty("--arrow-position", arrowPosition);
                })();

                // Styles supplémentaires pour le popup
                l.style.visibility = "inherit";
                l.style.overflow = "initial";
                l.style.color = "white";
                l.classList.add("tooltipes_");
                l.style.zIndex = "1044";
                l.style.boxShadow = "2px 2px 7px 2px rgba(0, 0, 0, .4)";
                ...
                // Ajouter la flèche uniquement avec border-top ou border-bottom
                const style = document.createElement("style");
                style.innerHTML = `
    .tooltipes_::after {
    content: '';
    ...
    border-left: 6px solid transparent;
    border-right: 6px solid transparent;
    border-top: ${l.style.getPropertyValue('--arrow-position') === 'bottom' ? '6px solid white' : 'none'};
    border-bottom: ${l.style.getPropertyValue('--arrow-position') === 'top' ? '6px solid white' : 'none'};
    top: ${l.style.getPropertyValue('--arrow-position') === 'top' ? '-6px' : 'calc(100% + 2px)'};
    }
    `;
                document.head.appendChild(style);
    ...
    function showVV() {
        $(document).on("click", "button.btn-showVV", function() {
            const currentlot = $(this).attr("data-set")
            let urlvv = ''
            if (currentlot == 'Maison type 4') {
                urlvv = 'https://panoraven.com/en/embed/yPStw70P4l'
            } else if (currentlot == 'Maison type 3 évolutif') {
                urlvv = 'https://panoraven.com/en/embed/0Qfg7wbQo7'
            }

            const currentVV = '<iframe width="95%" class="iframevv" height="600px" allowFullScreen="true" allow="accelerometer; magnetometer; gyroscope" style="background-color: #FF; display:block; margin:20px auto; border:0 none; max-width:980px;border-radius:8px; box-shadow: 0 1px 1px rgba(0,0,0,0.11),0 2px 2px rgba(0,0,0,0.11),0 4px 4px rgba(0,0,0,0.11),0 6px 8px rgba(0,0,0,0.11),0 8px 16px rgba(0,0,0,0.11);" src="' +
                urlvv + '"></iframe>'
            const closeBtn = '<button type="button" class="close border p-0 rounded-circle bg-white" data-dismiss="modal" aria-label="Close" style="width: 3rem; height: 3rem;"><span aria-hidden="true" class="icon-span rounded-circle text-secondary" style="font-size: 2rem; line-height: 0;">&times;</span></button>'
            const loading = '<p class="text-danger h2">Loading...</p>'
            $('div#vvModal').modal('show')
            $('div#vvModal .modal-header').html(closeBtn)
            $('div#vvModal .modal-body').html(currentVV, loading)
        })
    }

    function ShowModal() {
        $(document).on("click", "button.btn-show", function() {
            const categ = $(this).attr("data-cat")
            const currentlot = $(this).attr("data-set")
            const extImage = categ == '2' ? '.jpg' : '.png'
            const currentImage = "<img src='assets/m4b/lot-" + categ + "d/" + currentlot + extImage + "' id='imageWithSelection' class='img-lot img w-100' /> ";
            const rangeZ = '<input type="range" min="0" max="6" step="1" value="0" class="w-25" id="rangezoom">'
            const closeBtn = '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true" class="icon-span rounded-circle">&times;</span></button>'
            $('#exampleModal').modal('show')
            $('#exampleModal .modal-content').css({
                'border': '2px solid #5B8BB7',
                'border-radius': '13px',
                'background-color': 'white'
            })
            $('#exampleModal .modal-header').css({
                'border-radius': '13px 13px 0px 0px',
                'background-color': 'white'
            }).html(closeBtn)
            $('#exampleModal .modal-title').html('')
            $('#exampleModal .icon-span').css({
                'color': '#5B8BB7',
                'border': '2px solid #5B8BB7',
                'display': 'block',
                'width': '3rem',
            ...
            $('#exampleModal .modal-body').html(currentImage)
            $('#exampleModal .modal-footer').html(rangeZ)
        })

        $(document).on("change", "input#rangezoom", function() {
    ...
    function ec() {
        ...
        var img = document.getElementById("boussole");
        img.style.transform = "rotateX(0deg) rotateZ(" + (298 + (f.I * 360) / 25) + "deg)";
        if (ca) {
            if (((ya = !1), 0 <= f.I && 0 <= f.J)) {
                (b < D || g < E) &&
                Ha > n &&
                ...
                if (0 < f.l.length) {
                    var y = 0,
                        h;
                    for (
                        pwb = b * Math.sqrt(window.devicePixelRatio || 1); f.l.length >= y + 2 && f.l[y + 1].width > pwb;
    ...
    c.changePolygonMode = function(a, e) {
        // console.log('change polygone mode', h, c, e, a);
        h.mode = 1 == e && 0 < h.mode ? 0 : Math.round(a);
        c.a = !0;
    };
    //hide Hotspot
    // creating new field for surface filter
    c.toShow = null;
    c.inactiveHotspots = [];
    c.hideHotspot = function(state, typo) {
        if (state == "hide") {
            c.inactiveHotspots.push(typo);
        } else {
            c.inactiveHotspots.splice(c.inactiveHotspots.indexOf(typo));
        }
    };
    //hide Hotspot
    ...
    // Ka = function (a, e) {
    //   var b = h.v;
    //   b.enabled && (c.hotspot != c.emptyHotspot && 0 <= a && 0 <= e && "" != c.hotspot.title
    //     ? ((l.innerHTML = c.hotspot.title),
    //       (l.style.color = S(b.U, b.ea)),
    //       b.background ? (l.style.backgroundColor = S(b.B, b.G)) : (l.style.backgroundColor = "transparent"),
    //       (l.style.border = "solid " + S(b.C, b.H) + " " + b.S + "px"),
    //       (l.style.borderRadius = b.R + "px"),
    //       (l.style.textAlign = "center"),
    //       0 < b.width ? ((l.style.left = a - b.width / 2 + "px"), (l.style.width = b.width + "px")) : ((l.style.width = "auto"), (l.style.left = a - l.offsetWidth / 2 + "px")),
    //       (l.style.height = 0 < b.height ? b.height + "px" : "auto"),
    //       (l.style.top = e + 25 + "px"),
    //       (l.style.visibility = "inherit"),
    //       (l.style.overflow = "hidden"))
    //     : ((l.style.visibility = "hidden"), (l.innerHTML = "")));
    //   console.log([c.hotspot.title]);
    // };
    c.ma = function() {
    ...
    c.createLayers = function(a) {
        ...
        c.W ?
            ((c.W.innerHTML = ""),
                ...
                l.setAttribute(
                    "style",
                    "top:\t0px;left: 0px;position:absolute;padding: 3px;visibility: hidden;z-index: 1040;"
                ),
            ...
            if ("view" == a.nodeName)
                for (e = a.firstChild; e;)
                    ...
                    ((b = e.getAttributeNode("column")),
                        ...
                        (f.y = f.$),
                        (b = e.getAttributeNode("state")),
                        (f.da = Number(b ? b.nodeValue : 0)),
                        (f.state = f.da)),
                    "zoom" == e.nodeName &&
                    ((b = e.getAttributeNode(screen.width < 912 ? "mobmin" : "min")),
                        (Qa = 1 * (b ? b.nodeValue : 1)),
                        (b = e.getAttributeNode(
                            screen.width < 912 ? "mobdefault" : "default"
                        )),
            ...
            if ("hotspots" == a.nodeName)
                for (e = a.firstChild; e;) {
                    ...
                    if ("polyhotspot" == e.nodeName) {
                        ...
                        (g.title = b.nodeValue.toString());
                        (b = e.getAttributeNode("backgroundcolor")) &&
                        (g.backgroundcolor = b.nodeValue.toString());
                        (b = e.getAttributeNode("backgroundalpha")) &&
                        (g.backgroundalpha = b.nodeValue.toString());
                        (b = e.getAttributeNode("borderalpha")) &&
                        (g.borderalpha = b.nodeValue.toString());
                        (b = e.getAttributeNode("bordercolor")) &&
                        (g.bordercolor = b.nodeValue.toString());
                        (b = e.getAttributeNode("id")) && (g.id = b.nodeValue.toString());
                        if ((b = e.getAttributeNode("reuse"))) g.reuse = b.nodeValue;
                        g.C = h.C;
                        g.B = h.B;
