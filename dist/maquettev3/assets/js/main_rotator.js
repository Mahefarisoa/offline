...
function object2vrPlayer(A) {
    ...
    function Jb(a) {
        ...
        /* =========================
           1. Velo
        ========================== */
        const veloIds = ['Vélo 01', 'Vélo 02', 'Vélo 03']; // adapte ici
        const isVelo = veloIds.includes(a.id);

        /* =========================
   1. PLACE
========================== */
const placeIds = [
    'Place 01', 'Place 02', 'Place 03', 'Place 04',
    'Place 05', 'Place 06', 'Place 07'
  ]; // adapte selon ton projet
  
  const isPlace = placeIds.includes(a.id);

        /* =========================
           1. DÉFINITION DES GARAGES
        ========================== */
        const garageIds = ['Garage 01', 'Garage 02', 'Garage 03', 'Garage 04', 'Garage 05', 'Garage 06', 'Garage 07', 'Garage 08',
            'Garage 09', 'Garage 10', 'Garage 11', 'Garage 12', 'Garage 13', 'Garage 14', 'Garage 15',
            'Garage 16', 'Garage 17', 'Garage 18', 'Garage 19', 'Garage 20', 'Garage 21', 'Garage 22', 'Garage 23',
            'Garage 24', 'Garage 25', 'Garage 26', 'Garage 27','Garage 28', 'Garage 29', 'Garage 30', 'Garage 31',
            'Garage 32', 'Garage 33', 'Garage 34'
        ]; // adapte ici
        const isGarage = garageIds.includes(a.id);
    
        /* =========================
           2. STATUS DISPO / INDISPO
        ========================== */
        const indispoStat = [];
        ...
        /* =========================
           3. BOUTONS LOT
        ========================== */
        let displayClass3D = 'd-none';
        let displayClass2D = 'd-none';
    
        if (['106', '204', '301', '402'].includes(a.id)) {
        ...
        if (isGarage) {
            /* -------- POPUP GARAGE -------- */
            l.innerHTML = `
            <div class="garage-popup" data-set="${a.id}" style="
                 display:flex;
                 align-items:center;
                 gap:6px;
                 padding:0 10px;
              ...
              <img 
                src="assets/img/garage-icon.svg"
                alt="Garage"
                style="width:20px; height:20px; margin-right: 6px;"> <!-- ⬅ ici margin-right -->
          
              <span class="garage-label" style="color:#003b5c;">${a.id}</span>
            </div>
          `;
          l.classList.add('garage-tooltip');

        
} else if (isVelo) {
    /* -------- POPUP VELO -------- */
    l.innerHTML = `
      <div class="velo-popup" data-set="${a.id}" style="
           display:flex;
           align-items:center;
           gap:6px;
           padding:0 10px;
        ...
        <img 
          src="assets/img/velo-icon.png"
          alt="Vélo"
          style="width:20px; height:20px; margin-right:6px;">
    
        <span class="velo-label" style="color:#003b5c;">${a.id}</span>
      </div>
    `;
    l.classList.add('velo-tooltip');

} else if (isPlace) {
    /* -------- PLACE -------- */
    l.innerHTML = `
      <div class="place-popup" data-set="${a.id}" style="
           display:flex;
           align-items:center;
           gap:6px;
           padding:0 12px;
        ...
        <img 
          src="assets/img/place-icon.png"
          alt="Place"
          style="width:18px; height:18px; margin-right:6px;">

        <span class="place-label" style="color:#003b5c;">
          ${a.id}
        </span>
      </div>
    `;
    l.classList.add('place-tooltip');
    l.classList.remove('garage-tooltip', 'velo-tooltip');

        } else {
            /* -------- POPUP LOT -------- */
            l.innerHTML = `
                <div class="bg-blue-marine pb-0" style="border-radius:8px;">
                    <span class="h4">${a.target}&nbsp;</span>
                    <span style="font-size:.8rem">Lot ${a.id}</span>
                    <p class="mb-0 text-uppercase text-white bg-${lStatusClass} pb-2 pt-2" style="font-size:.8rem">
                        ${lStatus}
                    </p>
                </div>
                <div class="bg-white mt-1" style="border-radius:8px;">
                    <button class="btn w-75 mb-2 mt-2 ${displayClass2D}"
                        data-cat="2" data-set="${a.id}" style="font-size:.9rem;">2D</button>
    
                    <button class="btn w-75 mb-2 ${displayClass3D}"
                        data-cat="3" data-set="${a.id}" style="font-size:.9rem;">3D</button>
                </div>
            `;
            l.classList.remove('garage-tooltip');
            l.classList.remove('velo-tooltip');
        }
    
        /* =========================
           5. STYLE GÉNÉRAL POPUP
        ========================== */
        ...
        l.style.boxShadow = "2px 2px 7px rgba(0,0,0,.4)";
        ...
        const popupHeight = rect.height || 80;
    
        let topPosition = my - popupHeight;
        ...
        const leftPosition = mx - (l.offsetWidth / 2);
        ...
        if (!document.getElementById('tooltip-arrow-style')) {
            ...
            style.innerHTML = `
                .tooltipes_::after {
                    ...
                    border-top: var(--arrow-position) == bottom ? 6px solid white : none;
                }
                .tooltipes_[style*="--arrow-position: top"]::after {
                    border-bottom:6px solid white;
                    top:-6px;
                }
                .tooltipes_[style*="--arrow-position: bottom"]::after {
    ...
    function Ka(a, e) {
        if (c.inactiveHotspots.indexOf(c.hotspot.target) >= 0) return;
    
        var b = h.v;
    
        if (!(b.enabled && c.hotspot !== c.emptyHotspot && a >= 0 && e >= 0 && c.hotspot.title !== "")) {
            l.style.visibility = "hidden";
            l.innerHTML = "";
            return;
        }
        ...
        ========================== */
        const veloIds = ['Vélo 01', 'Vélo 02', 'Vélo 03']; // adapte ici
        const isVelo = veloIds.includes(c.hotspot.title);
...
const placeIds = [
  ...
  const isPlace = placeIds.includes(c.hotspot.title);
        ...
        const garageIds = ['Garage 01', 'Garage 02', 'Garage 03', 'Garage 04', 'Garage 05', 'Garage 06', 'Garage 07', 'Garage 08',
            ...
            'Garage 16', 'Garage 17', 'Garage 18', 'Garage 19', 'Garage 20', 'Garage 21', 'Garage 22', 'Garage 23',
            'Garage 24', 'Garage 25', 'Garage 26', 'Garage 27','Garage 28', 'Garage 29', 'Garage 30', 'Garage 31',
            'Garage 32', 'Garage 33', 'Garage 34']; // adapte ici
        const isGarage = garageIds.includes(c.hotspot.title);
        ...
        if (['106', '204', '301', '402'].includes(c.hotspot.title)) {
            displayClass3D = 'btn-outline-primary btn-show';
        }
        ...
        if (isGarage) {
            /* -------- GARAGE -------- */
l.innerHTML = `
  <div class="garage-popup" data-set="${c.hotspot.title}" style="
       display:flex;
       align-items:center;
       gap:6px;
       padding:0 10px;
    ...
    <span class="garage-label" style="color:#003b5c;">${c.hotspot.title}</span>
  </div>
`;
l.classList.add('garage-tooltip');
    ...
    /* -------- POPUP VELO -------- */
    l.innerHTML = `
      <div class="velo-popup" data-set="${c.hotspot.title}" style="
           display:flex;
           align-items:center;
           gap:6px;
           padding:0 10px;
        ...
        <span class="velo-label" style="color:#003b5c;">${c.hotspot.title}</span>
      </div>
    `;
    l.classList.add('velo-tooltip');
    ...
    /* -------- PLACE -------- */
    l.innerHTML = `
      <div class="place-popup" data-set="${c.hotspot.title}" style="
           display:flex;
           align-items:center;
           gap:6px;
           padding:0 12px;
    ...
    l.classList.remove('garage-tooltip', 'velo-tooltip');
            ...
            /* -------- LOT -------- */
            l.innerHTML = `
                <div class="bg-blue-ciel pb-0" style="border-radius:8px;">
                    <span class="h4">${c.hotspot.target}&nbsp;</span>
                    <span style="font-size:.8rem">Lot ${c.hotspot.title}</span>
                    <p class="mb-0 text-uppercase text-white bg-${lStatusClass} pb-2 pt-2" style="font-size:.8rem">
                        ${lStatus}
                    </p>
                </div>
                <div class="bg-white mt-1">
                    <button class="btn w-75 mb-2 mt-2 ${displayClass2D}"
                        data-cat="2" data-set="${c.hotspot.title}" style="font-size:.9rem;">2D</button>
    
                    <button class="btn w-75 mb-2 ${displayClass3D}"
                        data-cat="3" data-set="${c.hotspot.title}" style="font-size:.9rem;">3D</button>
                </div>
            `;
            l.classList.remove('garage-tooltip');
            l.classList.remove('velo-tooltip');
        ...
        if (b.width > 0) {
            ...
            l.style.width = isGarage ? "auto" : (b.width / 1.4) + "px";
        } else {
